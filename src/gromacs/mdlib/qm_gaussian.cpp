...
void QMMM_QMgaussian::init_gaussian(QMMM_QMsurfaceHopping &surfaceHopping, int QMbasis)
{
...
static void write_gaussian_SH_input(int step, bool swap, QMMM_QMrec& qm, QMMM_MMrec& mm)
{
    bool  bSA = (qm.surfaceHopping.SAstep > 0);
    ...
    /* local version */
    fprintf(out, "%s%s%s", "%subst l510 ", qm.gaussian.devel_dir, "/l510\n");
    fprintf(out, "%s%s%s", "%subst l301 ", qm.gaussian.devel_dir, "/l301\n");
    fprintf(out, "%s%s%s", "%subst l701 ", qm.gaussian.devel_dir, "/l701\n");

    fprintf(out, "%s%s%s", "%subst l1003 ", qm.gaussian.devel_dir, "/l1003\n");
    ...
    if (mm.nrMMatoms)
    {
        fprintf(out, " 3/5=%d,6=%d,7=%d,25=1,32=1,43=1,94=-2/1,2,3;\n",
                qm.surfaceHopping.SHbasis[0], qm.surfaceHopping.SHbasis[1], qm.surfaceHopping.SHbasis[2]); /*basisset stuff */
    }
    else
    {
    ...
    {             /* hack, to alyays read from chk file!!!!! */
        fprintf(out, "%s%d,%s%d%s", " 4/5=1,7=6,17=", qm.surfaceHopping.CASelectrons,
                "18=", qm.surfaceHopping.CASorbitals, "/1,5;\n");
    }
    else /* generate the first checkpoint file */
    {
        fprintf(out, "%s%d,%s%d%s", " 4/5=0,7=6,17=", qm.surfaceHopping.CASelectrons,
                "18=", qm.surfaceHopping.CASorbitals, "/1,5;\n");
    }
    ...
    else
    {
        ...
        if (qm.QMmethod_get() >= eQMmethodRHF)
        {
            if (qm.QMmethod_get() == eQMmethodCASSCF)
            {
                 ...
                 */
                fprintf(out, "(%d,%d)", qm.surfaceHopping.CASelectrons, qm.surfaceHopping.CASorbitals);
            }
...
static real read_gaussian_SH_output(rvec QMgrad[], rvec MMgrad[], int step, QMMM_QMrec& qm, QMMM_MMrec& mm)
{
    ...
    else
    {
         ...
         */
        for (int i = 0; i < qm.surfaceHopping.CIdim; i++)
        {
...
real QMMM_QMgaussian::call_gaussian_SH(QMMM_QMrec& qm, QMMM_MMrec& mm, rvec f[], rvec fshift[])
{
    ...
    int         state;
    real        QMener  = 0.0;
    static bool swapped = FALSE; /* handle for identifying the current PES */
    bool        swap    = FALSE; /* the actual swap */
    rvec       *QMgrad, *MMgrad;
    char*       buf;
    char*       exe;
    ...
    write_gaussian_SH_input(step, swapped, qm, mm);
    ...
    fprintf(stderr, "step %5d, SA = %5d, swap = %5d\n", step,
            static_cast<int>(qm.surfaceHopping.SAstep > 0), static_cast<int>(swapped));
    step++;
    free(exe);
    return (QMener);
