                           ...
                           gmx_wallcycle_t wcycle)
{
    ...
    #pragma omp parallel
    {
        #pragma omp single
        {
            ...
            // Task for TensorFlow calculation with zero charge
            #pragma omp task shared(e_delta, f_delta, qm_zero_charge)
            {
                // Call TensorFlow for delta correction using zero-charge qm (using f_delta)
                e_delta = call_tensorflow(qr, cr, qm_zero_charge, mm, f_delta, fshift, nrnb, wcycle);
            }
