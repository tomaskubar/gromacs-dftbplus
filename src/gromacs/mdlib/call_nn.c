...
void init_nn(QMMM_QMrec*       qm)
{
    ...
    qm->TF_Model->Graph = TF_NewGraph();
    qm->TF_Model->Status = TF_NewStatus();
    ...
    int ntags = 1;
    qm->TF_Model->Session = TF_LoadSessionFromSavedModel(SessionOpts, RunOpts, saved_model_dir, &tags, ntags, qm->TF_Model->Graph, NULL, qm->TF_Model->Status);
    if(TF_GetCode(qm->TF_Model->Status) == TF_OK)
    {
        printf("TF_LoadSessionFromSavedModel OK\n");
    }
    else
    {
        printf("%s",TF_Message(qm->TF_Model->Status));
        exit(-1);
    }

    //****** Get input tensor
    qm->TF_Model->NumInputs = 1;
    qm->TF_Model->Input = malloc(sizeof(TF_Output) * qm->TF_Model->NumInputs);
    TF_Output t0 = {TF_GraphOperationByName(qm->TF_Model->Graph, "serving_default_input_1"), 0};
    ...
    qm->TF_Model->Input[0] = t0;
    
    //********* Get Output tensor
    qm->TF_Model->NumOutputs = 1;
    qm->TF_Model->Output = malloc(sizeof(TF_Output) * qm->TF_Model->NumOutputs);
    TF_Output t2 = {TF_GraphOperationByName(qm->TF_Model->Graph, "StatefulPartitionedCall"), 0};
    ...
    qm->TF_Model->Output[0] = t2;

    qm->TF_Model->InputValues  = (TF_Tensor**)malloc(sizeof(TF_Tensor*)*qm->TF_Model->NumInputs);
    qm->TF_Model->OutputValues = (TF_Tensor**)malloc(sizeof(TF_Tensor*)*qm->TF_Model->NumOutputs);
    
    TF_DeleteSessionOptions(SessionOpts);
    TF_DeleteBuffer(RunOpts);
    
    /* Pass the list of QM atoms to DFTB+ */
    qm->TF_Model->nAtoms = qm->nrQMatoms_get();
    snew(qm->TF_Model->atomicNumbers, qm->TF_Model->nAtoms);
    
    // read the atomic numbers of QM atoms, and set up the lists
    for (int i=0; i<qm->TF_Model->nAtoms; i++)
    {
                   ...
                   gmx_wallcycle_t   wcycle)
{
 ...
 //  calcQMextPotPME(nullptr, nullptr, true, lPme, fr, cr, wcycle, fr->rcoulomb, fr->ewaldcoeff_q);
    ...
    qm->TF_Model->InputValues[0] = int_tensor;

    // Run the Session
    TF_SessionRun(qm->TF_Model->Session, NULL, qm->TF_Model->Input, qm->TF_Model->InputValues, qm->TF_Model->NumInputs, qm->TF_Model->Output, qm->TF_Model->OutputValues, qm->TF_Model->NumOutputs, NULL, 0,NULL , qm->TF_Model->Status);
